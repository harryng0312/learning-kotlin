...
protoc --plugin=protoc-gen-grpc-java=$GRPC_JAVA_GEN --grpc-java_out=$JAVA_OUTPUT_FILE --java_out=$JAVA_OUTPUT_FILE --proto_path=$DIR_OF_PROTO_FILE $PROTO_FILE
protoc --plugin=protoc-gen-grpckt=$GRPC_KT_GEN --grpckt_out=$KT_OUTPUT_FILE --kotlin_out=$KT_OUTPUT_FILE --proto_path=$DIR_OF_PROTO_FILE $PROTO_FILE